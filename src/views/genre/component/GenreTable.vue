<template>
  ...
  <table>
    ...
    <tbody>
      <tr class="genre-table" v-for="(item, index) in categories" :key="item.id" :class="[tableTrStyle, index % 2 === 0 ? tableTrOddStyle : '']">
        <checkbox-cell v-if="checkable" @checked="checked($event, genre)" />
        <td class="genre-table-id" data-label="Id">
          {{ item.id }}
        </td>
        <td class="genre-table-name" data-label="Name">
          {{ item.name }}
        </td>
          ...
          <div class="flex justify-end">
            <div class="px-2">
              <router-link :to="{name: 'genreEdit', params: { id: item.id}}">
                <button id="edit-genre-button" class="bg-green-500 hover:bg-green-700 text-white text-sm font-bold py-2 px-4 rounded">
                    Edit
                  </button>
              </router-link>
            </div>
            <div class="px-2">
              <router-link :to="{name: 'genre', params: { id: item.id}}">
                <button id="delete-genre-button" class="bg-red-500 hover:bg-red-700 text-white text-sm font-bold py-2 px-4 rounded" @click="deleteGenres(item.id)">
                    Delete
                  </button>
              </router-link>
            </div>
